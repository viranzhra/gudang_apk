                    ...
                    <tbody>
                        @foreach ($data as $d)
                            <tr>
                                ...
                                <td>{{ $d->jumlah }}</td>
                                <td>
                                    <button type="button" data-bs-toggle="modal" data-bs-target="#detailData"
                                        aria-label="Detail" data-id="{{ $d->id }}" class="btn-detail btn-action"
                                        style="border: none;">
                                        <iconify-icon icon="mdi:file-document-outline" class="icon-detail"></iconify-icon>
                                    </button>
                                    <button type="button" class="btn-edit btn-action" data-id="{{ $d->id }}"
                                        data-nama="{{ $d->nama }}" data-jenis="{{ $d->nama_jenis_barang }}"
                                        data-supplier="{{ $d->nama_supplier }}" data-keterangan="{{ $d->keterangan }}"
                                        aria-label="Edit">
            ...
            @else
                <div class="py-8 px-4 mx-auto max-w-screen-xl lg:py-16 lg:px-6">
                    <div class="mx-auto max-w-screen-sm text-center">
                        <p class="mb-4 text-3xl tracking-tight font-bold text-gray-900 md:text-4xl dark:text-white">Data
                            tidak ditemukan.</p>
                    </div>
                </div>
            @endif
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            document.querySelectorAll('[aria-label="Detail"]').forEach(button => {
                button.addEventListener('click', function() {
                    ...
                    const barangId = customerRow.querySelector('td:nth-child(3)').innerText
                        .trim();
                    const barangNama = customerRow.querySelector('td:nth-child(4)').innerText
                        .trim();
                    const barangJenis = customerRow.querySelector('td:nth-child(5)').innerText
                        .trim();
                    const barangSupplier = customerRow.querySelector('td:nth-child(6)')
                        .innerText.trim();
                    const barangKeterangan = customerRow.querySelector('td:nth-child(6)')
                        .innerText.trim();

                    // Populate the modal with customer details
                    document.getElementById('detail-nama').innerText = barangNama;
                    document.getElementById('detail-jenis').innerText = barangJenis;
                    document.getElementById('detail-supplier').innerText = barangSupplier;
                    document.getElementById('detail-keterangan').innerText = barangKeterangan;
