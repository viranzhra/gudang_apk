    ...
    <script>
        ...
        $('form[id="addForm"]').on('submit', function(e) {
            ...
            var formData = {
                _token: '{{ csrf_token() }}',
                nama: $('#nama').val(),
                nama_tanggal_akhir: $('#extend').is(':checked') ? $('#nama_tanggal_akhir').val() :
                null, // Send null if unchecked
                extend: $('#extend').is(':checked') ? '1' : '0' // Send '0' if unchecked
            };
            ...
            $.ajax({
                ...
                error: function(xhr) {
                    let message = xhr.responseJSON && xhr.responseJSON.errors ? xhr.responseJSON.errors
                        .nama[0] : 'An error occurred.';
